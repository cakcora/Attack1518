...
############################# Random Attacks  #####################
        ...
        for target_node in tqdm(node_list):
            ...
            random_node_list = random.sample(list(idx_unlabeled_all), len(perterbation_list))
            print(random_node_list)

            perterbation_list_final = [[Target_node, value] for value in random_node_list]

            print(perterbation_list_final)
            print(len(perterbation_list_final))
            ...
            for ep in range(n_perturbations):
                ...
                for i in perterbation_list_final:
                    modified_adj_chk_work = modified_adj_chk.copy().tolil()  ########## this will work as reset of the edge list
                    # print(modified_adj_chk_work[i[0], i[1]])
                ...
                print(sorted_list[0])
                rmv_del_edge.append(sorted_list[0])
                ...
                print(len(perterbation_list_final))
                list_of_lists = perterbation_list_final
                # list_of_lists = list_of_lists.tolist()
                list_to_remove = [sorted_list[0][0], sorted_list[0][1]]
                print(list_to_remove)
                filtered_list_of_lists = [x for x in list_of_lists if x != list_to_remove]
                # filtered_list_of_lists = [lst for lst in list_of_lists if lst not in list_to_remove]
                perterbation_list_final = filtered_list_of_lists  # np.array(filtered_list_of_lists)
                # perterbation_list=perterbation_list.tolist()
                print(len(perterbation_list_final))

                if len(perterbation_list_final) == 0:
                    break
                # print(len(filtered_list_of_lists))
        ...
        print('========= TESTING Random Connection =========')

        print('Missclassification rate for GCN : %s' % (cnt_gcn / num))
        ### print('Missclassification rate for GAT : %s' % (cnt_gat / num))

        miss[1][gp] = cnt_gcn / num
        # miss[1][gp] = cnt_gat / num

        df_ep = pd.DataFrame(rmv_del_edge)
        print(df_ep)

        filename_epsd = f"random_edge_cora.csv"
        df_ep.to_csv(filename_epsd, index=False)
...
######################## Netattacks  #####################
        ...
        print('Miss-classification rate GCN: %s' % (cnt_gcn / num))
        #print('Miss-classification rate GAT: %s' % (cnt_gat / num))

        miss[2][gp] = cnt_gcn / num
...
######################################### 1819  ###############################
        ...
        miss[6][gp] = cnt_gcn / num
        # miss[1][gp] = cnt_gat / num
        #####################################  Not same orbit connection but takes minimum number of nodes ##############################
        ...
        for target_node in tqdm(node_list):
            ...
            matching_indices_revised_final = random.sample(matching_indices_revised, len(matching_indices))

            perterbation_list = [[Target_node, value] for value in matching_indices_revised_final]
        ...
        print('========= TESTING Not Same Orbit Connection_not_all orbit_node =========')

        print('Missclassification rate for GCN : %s' % (cnt_gcn / num))
        ### print('Missclassification rate for GAT : %s' % (cnt_gat / num))

        miss[7][gp] = cnt_gcn / num
        # miss[1][gp] = cnt_gat / num
