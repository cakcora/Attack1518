...
def get_linearized_weight1(surrogate):
    conv1_parameters=list(surrogate.gc2.parameters())
    #conv1_parameters = list(surrogate.lin1.parameters())
    weigh_matrix1=conv1_parameters[0].data

    conv1_parameters2 = list(surrogate.gc3.parameters())
    #conv1_parameters2 = list(surrogate.lin2.parameters())
    weigh_matrix2 = conv1_parameters2[0].data

    W = weigh_matrix1 @ weigh_matrix2
    return W.detach().cpu().numpy()
...
def compute_logits1(work_modified_adj, modified_features, surrogate, target_node):
    adj_norm = normalize_adj(work_modified_adj)
    W = get_linearized_weight1(surrogate)
    logits = (adj_norm @ adj_norm @ modified_features @ W)[target_node]
    return logits
