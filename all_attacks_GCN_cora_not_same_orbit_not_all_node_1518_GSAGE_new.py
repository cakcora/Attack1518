...
def select_nodes(target_gcn=None):
    ...
    if target_gcn is None:
        target_gcn = GraphSAGE(nfeat=features.shape[1],
                  nhid=16,
...
def test_GAT(adj, data, target_node):
    ...
    gat = GraphSAGE(nfeat=features.shape[1], nhid=8, heads=8, nclass=labels.max().item() + 1, dropout=0.5, device=device)
    gat = gat.to(device)
...
#surrogate = GraphSAGE(nfeat=features.shape[1], nclass=labels.max().item()+1,nhid=16, dropout=0, with_relu=False, with_bias=False, device=device)
surrogate = GraphSAGE(nfeat=features.shape[1], nclass=labels.max().item()+1,nhid=16, dropout=0, device=device)
...
filenames = ['new_GSAGE_Cora_all.csv']
